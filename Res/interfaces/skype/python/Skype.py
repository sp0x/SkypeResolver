...
class Message(skypekit.Object):
  ...
  - emergency_call_denied 
    ...
    def _sk_get_leavereason(self):
        """
        leave reason for message of the RETIRED type
        """
        return skypekit.enumof(Message.LEAVEREASON, self._sk_property("ZG~]\011", 126, True))
...
class Video(skypekit.Object):
  ...
  """
    ...
    def OnLastFrameCapture(
        ...
        """
        pass
    event_handlers[1] = "_sk_on_last_frame_capture"
    def _sk_on_last_frame_capture(self, parms):
        """ internal event dispatcher for OnLastFrameCapture
  """
        l_cleanparms = (parms.get(1, '')),
        l_cleanparms += (parms.get(2, 0)),
        l_cleanparms += (parms.get(3, 0)),
        self.OnLastFrameCapture(*l_cleanparms)
...
class Account(skypekit.Object):
  ...
  - PWD_OK_BUT_CHANGE_SUGGESTED - consider displaying an updated indicator, along with a recommendation to change again to a stronger password. 
    ...
    def _sk_get_suggested_skypename(self):
        """
        suggested skypename. present if logoutreason==SKYPENAME_TAKEN
        """
