...
def main():
	...
	options = ["skypekit=", "videohost=", "voicehost=", "acspath=", "dbg=", "kitlog=", "videolog=", "voicelog=", "help", "valgrind", "instance="]
	acspath = None
	debuglogname = None
	kitlog = None
	videolog = None
	...
	if skypekit_binary == None:
		...
		help(options)
	elif (not skypekit_binary.startswith('/')) and (not skypekit_binary.startswith('./')):
		skypekit_binary = './' + skypekit_binary
	if not os.path.isfile(skypekit_binary):
		print 'invalid skypekit_binary specified: %s' %(skypekit_binary)
		help(options)
